...
![Version](https://img.shields.io/badge/version-3.0.2-8b0000?style=flat-square)
...
- **Two-part home** — a **Compendium** section (Clans, Disciplines) for reference, and **My Characters** with a row per character showing their clan sigil, name, clan and generation.
...
- **Responsive** — mobile-first layout, 1 → 5 columns on the discipline grid depending on viewport, full-screen overlay menu that matches the page width and steps its type up on desktop.
...
└── src/
    ...
    ├── App.vue               # Custom sticky navbar + sectioned overlay menu + page transitions
    ├── router.ts             # Hash routes
    ├── types.ts              # TypeScript interfaces (Discipline, Power, Clan, Character)
